...
def get_data_beaver(inv=False, eos=True, label="safe", ratio=None, seed=0):
    ...
    return train_dataset.map(reform).shuffle(seed=seed), eval_dataset.map(reform).shuffle(seed=0)
...
def train_dpo():
    ...
    for func, name in [
        ...
        if not script_args.not_save_model:
            ...
            if script_args.data_path in [name, 'none']:
                train_dataset.append(tr)
        elif script_args.data_path == "none":
            train_dataset.append(tr)
            eval_dataset.append(ev)
    ...
    training_args = TrainingArguments(
        ...
        # max_steps=math.ceil(script_args.num_train_epochs * total_length / script_args.gradient_accumulation_steps / script_args.per_device_train_batch_size),
        num_train_epochs=script_args.num_train_epochs,
        metric_for_best_model=None if script_args.not_save_model else 'rewards/true_accuracies_total',
